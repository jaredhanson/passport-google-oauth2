 ...
 *   - `parser`        an object containing a name and a parse(json) function if you want to consume another google API (ie. People API)
 *
 * Examples:
 *
...
function Strategy(options, verify) {
  ...
  if (options.profileParser) {
    ...
    this.profileParser[this._userProfileFormat] = options.profileParser;
  } else {
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get(this._userProfileURL, accessToken, function (err, body, res) {
    ...
    if (!parser) {
      return done(new ParserError('Parser not found for profile format '+ this._userProfileFormat));
    }
