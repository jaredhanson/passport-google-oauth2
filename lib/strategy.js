...
function Strategy(options, verify) {
  ...
  if (options.profileParser) {
    this._userProfileFormat = options.profileParser.name;
    this.profileParser[options.profileParser.name] = options.profileParser;
  } else {
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get(this._userProfileURL, accessToken, function (err, body, res) {
    ...
    var profile = self.getProfile(json);
...
Strategy.prototype.getProfile = function(json) {
  var parser = this.profileParsers[this._userProfileFormat];
  if (!parser) {
    return new ParserError('Parser not found for profile format '+ this._userProfileFormat);
  }
